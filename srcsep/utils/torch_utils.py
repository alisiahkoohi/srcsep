...
def is_long_tensor(z):
    return isinstance(z, torch.LongTensor) or isinstance(
        z, torch.cuda.LongTensor)


def is_double_tensor(z):
    return isinstance(z, torch.DoubleTensor) or isinstance(
        z, torch.cuda.DoubleTensor)
...
def multid_row_isin(a, b):
    ...
    a_isin = np.array(
        [x.tobytes() in d.keys() for x in a.reshape(-1, a.shape[-1])])
