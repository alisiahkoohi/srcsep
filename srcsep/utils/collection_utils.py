...
def transpose(li: Iterable[Iterable]) -> List[List]:
    """
    Transpose [[a11, ..., a1n], ... [am1, ..., amn]] and return  [[a11, ...,
    am1], ... [a1n, ..., amn]].
    """
...
def split_equal_sum(li: List[int],
                    r: int) -> Tuple[List[List[int]], List[List[int]]]:
...
def df_product(*dfs: pd.DataFrame) -> pd.DataFrame:
    for df in dfs:
        df['key'] = 1
    return reduce(lambda l, r: pd.merge(l, r, on='key'),
                  dfs).drop(columns='key')


def df_product_channel_single(df: pd.DataFrame, N: int,
                              method: str) -> pd.DataFrame:
    """ Pandas cartesian product {(0,0), ..., (0, Nl-1))} x df """
    if method == "same":
        df_n = pd.DataFrame(np.stack([np.arange(N), np.arange(N)], 1),
                            columns=['nl', 'nr'])
    elif method == "zero_left":
        df_n = pd.DataFrame(np.stack(
            [np.zeros(N, dtype=np.int32),
             np.arange(N)], 1),
                            columns=['nl', 'nr'])
    elif method == "zero_right":
        df_n = pd.DataFrame(np.stack(
            [np.arange(N), np.zeros(N, dtype=np.int32)], 1),
                            columns=['nl', 'nr'])
    else:
...
def df_product_channel_double(df: pd.DataFrame, Nl: int, Nr: int,
                              offdiag: bool) -> pd.DataFrame:
    ...
    df_prod = df_product(df_nl, df_nr, df)

    if offdiag:
        df_prod = df_prod[~(df_prod['nl'] == df_prod['nr'])]
